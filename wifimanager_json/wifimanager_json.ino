...
void loop(){
    ...
    HTTPClient http;
    http.begin(relay_server);
    int httpCode = http.GET();
    if (httpCode > 0) {
        digitalWrite(outputled, OFF);
        Serial.print("read data from server success ");
        Serial.println(relay_server);
        ...
        String getRelay1 = root["relay1"];
        String getRelay2 = root["relay2"];

        if ( getRelay1 == "1") {
            digitalWrite(outputrelay1, ON);
        ...
        digitalWrite(outputled, ON);
        Serial.print("read data from server failed ");
        Serial.println(relay_server);
    }
    http.end();
